...
public class DGraph {
    ...
    private Map<List<Integer>, Double> nodeGraph;
    ...
    public void addEdge(int v, int w, double distance) {
        ...
        edgeSet.add(v); edgeSet.add(w);
        
        nodeGraph.put(edgeSet, distance);
    ...
    /*
     * A method that finds the smallest edge in the map
     */
    public List<Integer> smallestVal() {
        return Collections
                .min(nodeGraph.entrySet(), Map.Entry.comparingByValue())
                .getKey();
    }
    ...
    public double getWeight(int v, int w) {
        ...
        if (nodeGraph.containsKey(edgeSet)) {
            return nodeGraph.get(edgeSet);
        } else {
            return -1;
        }
    }
